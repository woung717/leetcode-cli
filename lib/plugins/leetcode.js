...
var PuppeteerBrowsers = require('@puppeteer/browsers');
...
plugin.getRequest = function(url, opts, cb) {
  (async () => {
    const req = http2.get(url, opts, (res) => {
    ...
    req.on('error', (e) => log.error(e)); 
  })();
}

// update options with user credentials
plugin.setupHeaders = function(opts, user) {
...
plugin.makeOpts = function(url) {
  ...
  plugin.setupHeaders(opts, session.getUser());
...
plugin.getFavorites = function(cb) {
  ...
  plugin.getRequest(config.sys.urls.favorites, opts, (data) => {
    spin.stop();
...
plugin.signin = function(user, cb) {
  ...
  (async (user) => {
    ...
    const cacheDir = path.resolve(__dirname, '../../bin');
    const installedBrowser = await PuppeteerBrowsers.install({browser: 'chrome', buildId: '122.0.6261.111', cacheDir: cacheDir});
    const browser = await Puppeteer.launch({headless: false, executablePath: installedBrowser.executablePath});

    try {
      const [page] = await browser.pages();

      await page.setViewport({width: 1200, height: 800});
      await page.goto(config.sys.urls.login);

      await page.waitForResponse(isCN ? 'https://leetcode.cn/' : 'https://leetcode.com/', { timeout: 0 });
      // hack to make sure we have cookie at leetcode.com when login with 3rd party
      await page.waitForSelector('body', { timeout: 0 }); 

      const cookies = await page.cookies();

      user.sessionCSRF = cookies.find((c) => c.name === 'csrftoken').value;
      user.sessionId = cookies.find((c) => c.name === 'LEETCODE_SESSION').value;
    } finally {
      await browser.close();
      spin.stop();
    }
